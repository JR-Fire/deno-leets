...
export function parseCoordinatesLetters(input: string[]): number {
    ...
    for (let i = 0; i < l; i++) {
        ...
        while (ixS <= ixE && (isNaN(coordinates[i][0]) || isNaN(coordinates[i][1]))) {
            if (isNaN(coordinates[i][0])) {
                ...
                if (!isNaN(n1)) {
                    ...
                    const potentials = letterNumbers.entries().filter(ln => ln[0].charAt(0) === s);
                    potentials.forEach(ln => {
                        if (isNaN(coordinates[i][0])) {
                            const letters = ln[0];
                            if (letters.length <= line.length - ixS && line.slice(ixS).startsWith(letters)) {
            ...
            if (isNaN(coordinates[i][1])) {
                ...
                if (!isNaN(n2)) {
                    ...
                    //check if letterred number
                    const potentials = letterNumbers.entries().filter(ln => ln[0].charAt(ln[0].length - 1) === s);
                    potentials.forEach(ln => {
                        if (isNaN(coordinates[i][1])) {
                            const letters = ln[0];
                            if (ixE + 1 >= letters.length && line.slice(0, ixE + 1).endsWith(letters)) {
