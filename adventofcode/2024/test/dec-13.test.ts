...
test("to win most prizes costs 33427", () => {
    expect(tokensToWinMost(machines, 100)).toBe(33427);
});
...
test("best to win most prizes costs 33427", () => {
    expect(machines.map(m => bestTokensToWin(m)).reduce((s, t) => s + t, 0)).toBe(33427);
});
...
test("best to win counts the same for 8", () => {
    expect(bestTokensToWin(machines[8])).toBe(tokensToWin(machines[8], 100))
});
...
test("best to win most hard prizes costs 666", () => {
    const r = machines.map(m => {
        m.PrizeAt[0] += 10000000000000;
        m.PrizeAt[1] += 10000000000000;
        return m;
    }).map(m => bestTokensToWin(m)).reduce((s, t) => s + t, 0);
    expect(r).toBeGreaterThan(78422920875253);
    expect(r).toBe(91649162972270);
});
