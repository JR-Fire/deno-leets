...
function draw(m: string[][], guard: number[], op?: number[]): [isLoop: boolean, p: string[][]] {
    ...
    while (i >= 0 && i < n && j >= 0 && j < k) {
        ...
        if (p[i][j] === BLOCK) {
            ...
            //turn
            turn++;
            if (loop.find(e => e[0] === i && e[1] === j && e[2] === `${directions[stepIx][2]}`)) {
                //check for loop after turn
                const nextt = turn % 4
                const nexti = i + (+directions[nextt][0])
                const nextj = j + (+directions[nextt][1])
                if (nexti >= 0 && nexti < n && nextj >= 0 && nextj < k 
                    && (p[nexti][nextj] === `${directions[nextt][2]}` || p[nexti][nextj] === BLOCK)) {
                    isLoop = true;
                    break;
                }
            }
