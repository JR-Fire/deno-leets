...
export function antinodes(m: string[][]) {
  const { ants, locationMap }: {
    ants: Map<string, number[][]>;
    locationMap: string[][];
  } = makeAnts(m);
...
function countAntinodes(locationMap: string[][]) {
  return locationMap.reduce(
    (s, l) => s += l.filter((c) => c === ANTINODE).length,
    0,
  );
}
...
function inMap(x: number, y: number, locationMap: string[][]) {
  return y >= 0 && x >= 0 && y < locationMap.length &&
    x < locationMap[y].length;
}
