...
export function resultsimple(r: number, arg: number[]): boolean {
  ...
  do {
    ...
    const mult = arg.slice(0, i).length > 1
      ? arg.slice(0, i).reduce((s, a) => s *= a, 1)
      : 0;
    if (r === add + mult) {
      return true;
    }
...
export function resultreal(r: number, arg: number[]): boolean {
  for (let i = 0; i < arg.length; i++) {
    for (let j = i; j <= arg.length; j++) {
      ...
      const add = add1.reduce((s, a) => s += a, 0) +
        add2.reduce((s, a) => s += a, 0);
...
export function stupidsum(
  ...
  return stupidsum(arg.slice(1), target, current + arg[0]) ||
    stupidsum(arg.slice(1), target, current * arg[0]);
}
...
function stupiderSum(arg: number[], target: number, current: number): boolean {
  ...
  return stupiderSum(arg.slice(1), target, current + arg[0]) ||
    stupiderSum(arg.slice(1), target, current * arg[0]) ||
    stupiderSum(arg.slice(1), target, stupiderNum);
}
